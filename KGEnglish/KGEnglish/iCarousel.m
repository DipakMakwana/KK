...
- (BOOL)carousel:(__unused iCarousel *)carousel shouldSelectItemAtIndex:(__unused int)index { return YES; }
- (void)carousel:(__unused iCarousel *)carousel didSelectItemAtIndex:(__unused int)index {}
...
@property (nonatomic, assign) int previousItemIndex;
@property (nonatomic, assign) int numberOfPlaceholdersToShow;
@property (nonatomic, assign) int numberOfVisibleItems;
...
{
    int index = [[_itemViews allValues] indexOfObject:view];
    if (index != NSNotFound)
    {
...
{
    int index = [self indexOfItemView:view];
...
- (void)setItemView:(UIView *)view forIndex:(int)index
{
...
- (void)insertView:(UIView *)view atIndex:(int)index
{
    NSMutableDictionary *newItemViews = [NSMutableDictionary dictionaryWithCapacity:[_itemViews count] + 1];
    for (NSNumber *number in [self indexesForVisibleItems])
    {
...
- (void)depthSortViews
{
    for (UIView *view in [[_itemViews allValues] sortedArrayUsingFunction:(int (*)(id, id, void *))compareViewDepth context:(__bridge void *)self])
    {
...
- (CGFloat)offsetForItemAtIndex:(int)index
{
...
- (void)transformItemView:(UIView *)view atIndex:(int)index
{
    //calculate offset
...
- (UIView *)loadViewAtIndex:(int)index withContainerView:(UIView *)containerView
{
    [self pushAnimationState:NO];
...
- (UIView *)loadViewAtIndex:(int)index
{
...
- (void)loadUnloadViews
{
    ...
    int min = -(int)ceilf((CGFloat)_numberOfPlaceholdersToShow/2.0f);
    int max = _numberOfItems - 1 + _numberOfPlaceholdersToShow/2;
    int offset = self.currentItemIndex - _numberOfVisibleItems/2;
    if (!_wrapEnabled)
    {
...
- (int)minScrollDistanceFromIndex:(int)fromIndex toIndex:(int)toIndex
{
    int directDistance = toIndex - fromIndex;
    if (_wrapEnabled)
    {
        int wrappedDistance = MIN(toIndex, fromIndex) + _numberOfItems - MAX(toIndex, fromIndex);
...
- (void)scrollByNumberOfItems:(int)itemCount duration:(NSTimeInterval)duration
{
    if (duration > 0.0)
    {
        CGFloat offset = 0.0f;
...
- (void)scrollToItemAtIndex:(int)index duration:(NSTimeInterval)duration
{
...
{
    int index = [self indexOfItemView:itemView];
    CGFloat offset = [self offsetForItemAtIndex:index];
...
- (void)didScroll
{
    ...
    int currentIndex = roundf(_scrollOffset);
    int difference = [self minScrollDistanceFromIndex:_previousItemIndex toIndex:currentIndex];
    if (difference)
    {
...
- (int)viewOrSuperviewIndex:(UIView *)view
{
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gesture shouldReceiveTouch:(UITouch *)touch
{
    ...
    {
        //handle tap
        int index = [self viewOrSuperviewIndex:touch.view];
...
{
    int index = [self indexOfItemView:[tapGesture.view.subviews lastObject]];
        ...
        switch (panGesture.state)
        {
            ...
            case UIGestureRecognizerStateCancelled:
            {
                ...
                if (!_decelerating && (_scrollToItemBoundary || (_scrollOffset - [self clampedOffset:_scrollOffset]) != 0.0f))
                {
                    ...
                    {
                        int direction = (int)(_startVelocity / fabsf(_startVelocity));
...
- (void)mouseUp:(NSEvent *)theEvent
{
    if (!_didDrag)
    {
        ...
        for (UIView *view in [[[_itemViews allValues] sortedArrayUsingFunction:(int (*)(id, id, void *))compareViewDepth context:(__bridge void *)self] reverseObjectEnumerator])
        {
